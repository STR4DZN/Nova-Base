...
3. **Topologia Dirigida e Socketlib Exclusivo (G2-AUD-004)**: Chamadas de mutação e status queries utilizam RPC dirigido via Socketlib (`executeAsUser` diretamente para a autoridade primária). O canal de broadcast nativo (`game.socket`) não aceita comandos e não há fallback não-autenticado; clientes remotos sem Socketlib falham closed imediatamente com `DM_TRANSPORT_UNAVAILABLE`. Clientes secundários não recebem nem executam mutações broadcast.
