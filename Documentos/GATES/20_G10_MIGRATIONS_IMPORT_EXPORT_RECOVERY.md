# 20 — G10 — Migrations / Import-Export / Recovery
...
Especificar em nível executável tudo que deve ser feito no G10, como o código deve ficar ao terminar e qual condição permite seguir ao gate seguinte.
...
- guess semantic defaults
- silent destructive repair
- blind downgrade
...
|---|---|---|
| G10.1 | Schema/migration registry | incremental paths |
| G10.2 | MigrationPlan/dry run | impact/warnings/ambiguity |
| G10.3 | Snapshot/backup | manifest/checksum |
| G10.4 | Quarantine | raw payload/reason/source |
| G10.5 | Repair workflows | preview/audit |
| G10.6 | Export formats | backup vs share |
| G10.7 | Import/remapping | collision/dependency handling |
| G10.8 | Safe mode/boot recovery | read safe, block risky writes |
| G10.9 | Rebuild/restore | indexes/caches/restore validation |
| G10.10 | Legacy rehearsal | old fixtures→current |
...
src/migrations/*
src/import-export/*
src/storage/integrity/*
src/storage/recovery/*
src/ui/diagnostics/migrations/*
tests/{migrations,import-export,recovery}/*
```
...
- [ ] incremental/idempotent migrations
- [ ] backup critical before destructive migration
- [ ] ambiguity quarantined
- [ ] no identity by name
- [ ] import preview
- [ ] ID remap consistent
- [ ] share export sanitized
- [ ] restore tested
- [ ] future schema rejected/read-only
- [ ] one corrupt record não destrói whole world
