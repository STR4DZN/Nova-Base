# 17 — G7 — Requests / Missions / Knowledge / Secrets
...
Especificar em nível executável tudo que deve ser feito no G7, como o código deve ficar ao terminar e qual condição permite seguir ao gate seguinte.
...
- Secret/disclosure
- ProjectionService
- player/GM projections
- approval queue UI

## O que NÃO pertence a este gate

- Request mutar diretamente
- Knowledge virar world truth
- CSS esconder secrets
- Mission virar Project
...
|---|---|---|
| G7.1 | Request model/immutability after submit | intent preserved |
| G7.2 | Review/decision/handlers | Commands/Plans only |
| G7.3 | Request→Mission idempotency | stable result refs |
| G7.4 | Mission objectives/stages/outcome | independent lifecycle |
| G7.5 | Knowledge records | belief/confidence/provenance |
| G7.6 | Secrets/disclosure | hidden/hinted/partial/full |
| G7.7 | ProjectionService | authority-side omission |
| G7.8 | Search/count leak prevention | indexes sanitized |
| G7.9 | Inbox/Mission/Knowledge UI | split view/queue |
| G7.10 | Leakage acceptance | payload-level verification |
...
src/secrets/*
src/projection/*
src/ui/domain-patterns/{requests,missions,knowledge,secrets}/*
tests/{requests,missions,knowledge,secrets,projection}/*
```
...
- [ ] submitted payload imutável
- [ ] approved ≠ fulfilled
- [ ] Mission independente
- [ ] Knowledge ≠ truth
- [ ] secret field não chega ao player
- [ ] counts/search/sort não vazam existência
- [ ] sharing respeita disclosure
- [ ] GM/player payloads diferentes corretamente
